...
class ReportFormApp:
    ...
    def create_widgets(self):
        ...
        self.question_1_layout = self.create_question_1_layout()
    ...
    def apply_style(self):
        self.window.setWindowIcon(QIcon(resource_path("data\\images\\argest.png")))
    ...
    def create_info_layout(self):
        ...
        for i, q in enumerate(("date", "hour", "place")):
            ...
            text[q].setAlignment(QtCore.Qt.AlignLeft)
        ...
        calendar_button.setCursor(QtCore.Qt.PointingHandCursor)
    ...
    def create_injury_layout(self):
        ...
        checkboxes_layout.setAlignment(QtCore.Qt.AlignCenter)
        ...
        injury_5.setAlignment(QtCore.Qt.AlignCenter)
    ...
    def create_body_layout(self):
        ...
        body_36 = QLabel(text)
        body_36.setAlignment(QtCore.Qt.AlignCenter)
    ...
    def add_company_logo(self):
        ...
        if logo_path:
            ...
            delete_logo.setCursor(QtCore.Qt.PointingHandCursor)
    ...
    def export(self):
        ...
        if self.situation_input.text() != "":
            self.answer["situation"] = self.situation_input.text()
        self.answer["description"] = self.description_box.toPlainText()
        self.answer["comments"] = self.comment_box.toPlainText()

        options = QFileDialog.Options()
        options |= QFileDialog.ShowDirsOnly

        # Utilisation d'un widget parent fictif (QWidget)
        dummy_parent = QWidget()
        dummy_parent.hide()

        default_filename = self.texts[self.language]['title']+'_'+ "-".join(self.answer['date'].split('/')) + ".pdf"
        initial_dir = ""
        file_path, _ = QFileDialog.getSaveFileName(dummy_parent,
        ...
        if file_path:
            folder_path = os.path.dirname(file_path)  # Chemin du dossier
            filename = os.path.basename(file_path)    # Nom du fichier choisi
            ...
            for key in ('category', 'report type', 'situation', 'injury'):
                if self.answer[key] == '':
...
class PDFGenerator:
    ...
    def create_pdf(self):
        ...
        self.story.append(self.create_header(1, total_pages))
    ...
    def create_header(self, page_number, total_page):
        # Header
    ...
    def create_situation(self):
        checkboxes_data = []
        if self.answer['situation'] in self.texts[self.language].keys():
            resp, text = int(self.answer['situation'].split('_')[1]), ''
        elif self.answer['situation'] == 'None' : resp = 0
        else: resp, text = 30, ' '+self.answer['situation']
        ...
        for i in range(1, 31):
            ...
            checkboxes_data[y].append(self.full_box if resp == i else self.empty_box)
            if resp+i == 60:
    ...
    def create_injury_body_comment(self, width=1):
        ...
        if width == 1:
            ...
            if len(self.answer['organs']) > 10:
                injury_table = Table([[Paragraph('<u>'+self.texts[self.language]['injury_0']+'</u>', self.subtitle_style)], [checkboxes_table]], colWidths=[self.half])
                col1_table = Table([[injury_table], [Paragraph('<u>'+self.texts[self.language]['injury_5']+'</u> '+self.answer['comments'], self.base_style)]], colWidths=[self.half])
                col1_table.setStyle(TableStyle([('LINEABOVE', (-1, -1), (-1, -1), 1, colors.black)]))

                organs_table = Table([[Paragraph('<u>'+self.texts[self.language]['pdf_body']+'</u>', self.subtitle_style)], [Paragraph(", ".join([self.texts[self.language][organ] for organ in self.answer['organs']]), self.base_style)]], colWidths=[self.quarter-10])

                main_table = Table([[col1_table, organs_table, Image(resource_path("data\\images\\body_capture.png"), width=self.quarter, height=730*(self.quarter/505))]], colWidths=[self.half, self.quarter-10, self.quarter+10])
                main_table.setStyle(TableStyle([
                ('VALIGN', (0, 0), (0, 0), 'MIDDLE'),
                ('VALIGN', (1, 0), (1, 0), 'TOP'),
                ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
